...
def submit_video(
    ...
    if (
        request.count_interval_secs is not None
        and request.count_interval_secs < 30
    ):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
...
def terminate_job(
    video_uuid: str, authenticated: bool = Depends(check_api_key)
):
    db_client = NxsDbFactory.create_db(
        NxsDbType.MONGODB,
        uri=args.cosmosdb_conn_str,
...
def get_videos(
    status: RequestStatus, authenticated: bool = Depends(check_api_key)
):
    db_client = NxsDbFactory.create_db(
        NxsDbType.MONGODB,
        uri=args.cosmosdb_conn_str,
...
def get_counts(
    ...
    for r in query_results:
        tracking_result = TrackingCountResult(
            timestamp=r["timestamp"] / 1000.0, counts=[]
        )

        counts = r["counts"]
        for roi_idx, roi_result in enumerate(counts):
            tracking_result_per_roi = TrackingCountPerRoi(
                roi_idx=roi_idx, counts=[]
            )
...
def get_visualization(
    ...
    return VisualizationResult(
        from_ts=start_ts, to_ts=end_ts, visualized_frames=urls
    )


def generate_blobstore_sas_url(
    account_name, container_name, blob_path, sas_token
):
