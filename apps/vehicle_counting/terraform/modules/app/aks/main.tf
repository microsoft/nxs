...
resource "azurerm_public_ip" "nxsapp_public_ip" {
  ...
  domain_name_label = "nxsapp-${var.base.deployment_name}-${random_password.nxs_dns_suffix.result}"
...
output aks_info {
  value = {
    ...
    domain_name_label = azurerm_public_ip.nxsapp_public_ip.domain_name_label
