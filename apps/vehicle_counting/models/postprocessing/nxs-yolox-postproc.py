...
def postprocessing(
    ...
    with_p6 = postproc_params.get("with_p6", False)

    score_thresh = float(postproc_params.get("score_thresh", 0.25))
    nms_thresh = float(postproc_params.get("nms_thresh", 0.45))
    ...
    predictions = postprocess(output, [model_h, model_w], p6=with_p6)[0]
    ...
    dets = multiclass_nms(boxes_xyxy, scores, nms_thr=nms_thresh, score_thr=score_thresh)
    ...
    if dets is not None:
        ...
        for bbox, score, cls_idx in zip(
            final_boxes, final_scores, final_cls_inds
        ):
            if score < score_thresh:
                continue

            left, top, right, bottom = np.array(bbox, dtype=np.int32)
            cls_idx = int(cls_idx)

            class_name = f"class_{cls_idx}"
            if len(classes) > cls_idx:
            ...
            object_list.append(
                {
                    ...
                    "rel_bbox": {
                        "left": float(left) / img_w,
                        "top": float(top) / img_h,
                        "right": float(right) / img_w,
                        "bottom": float(bottom) / img_h,
                    },
